...
// 💡 1. 렌더링될 때마다 재생성되지 않도록 고정 데이터를 컴포넌트 외부로 분리
const SIDEBAR_INFO = {
  action: {
    title: "사고대처",
    desc: "교통사고 발생 시\n단계별 대처 가이드입니다.",
...
// 💡 2. 모달 컴포넌트를 분리하여 JSX를 깔끔하게 유지
const PolicyModal = ({ isOpen, onClose, info }) => {
  if (!isOpen) return null;

  return createPortal(
      <div className="policy-modal-overlay">
        <div className="policy-modal-content">
          <h3 className="policy-modal-header">
            🛡️ {info.title} 운영정책
          </h3>
          ...
          <div className="policy-modal-footer">
            <button onClick={onClose} className="policy-modal-close-btn">
              닫기
            </button>
          </div>
        </div>
...
export default function LeftSidebar({ activeTab }) {
  ...
  return (
      <aside className="left-sidebar">
        ...
        {/* 카페 카테고리 스타일 메뉴 영역 */}
        <div className="cafe-style-menu">
          <div className="cafe-menu-group">
            <Link to="/guides" className={`cafe-main-link ${activeTab === "action" ? "active" : ""}`}>
              <span className="icon"><PiSirenBold color="#f32222" size={18}/></span> 사고 대처
            </Link>
          </div>

          <div className="cafe-menu-group">
            <Link to="/cases" className={`cafe-main-link ${activeTab === "type" ? "active" : ""}`}>
              <span className="icon"><FaCarSide color="#f7984b" size={18} /></span> 사고 유형
            </Link>
            <ul className="cafe-sub-list">
              <li><Link to="/cases/car-to-car">차량 대 차량</Link></li>
              <li><Link to="/cases/car-to-pedestrian">차량 대 보행자</Link></li>
              <li><Link to="/cases/car-to-motorcycle">차량 대 이륜차</Link></li>
              <li><Link to="/cases/car-to-bicycle">차량 대 자전거</Link></li>
              <li><Link to="/cases/single-accident">단독사고</Link></li>
              <li><Link to="/cases/etc-accident">기타사고</Link></li>
            </ul>
          </div>

          <div className="cafe-menu-group">
            <Link to="/fault-ratios" className={`cafe-main-link ${activeTab === "ratio" ? "active" : ""}`}>
              <span className="icon"><FaBalanceScale color="#4437ff" size={18} /></span> 과실비율 조회
            </Link>
          </div>

          <div className="cafe-menu-group">
            <Link to="/community" className={`cafe-main-link ${activeTab === "community" ? "active" : ""}`}>
              <span className="icon"><HiMiniChatBubbleLeftEllipsis size={18} style={{ filter: "invert(1)" }} /></span> 커뮤니티
            </Link>
            <ul className="cafe-sub-list">
              <li><Link to="/community/free">자유게시판</Link></li>
              <li><Link to="/community/qna">QnA</Link></li>
              <li><Link to="/community/reviews">사고 후기</Link></li>
            </ul>
          </div>
        </div>
